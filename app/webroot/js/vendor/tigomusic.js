...
/*DZ.Event.subscribe('player_buffering', function(arg) {
	...
	DZ.Event.subscribe('current_track', function(arg) {
		...
		DZ.Event.subscribe('player_buffering', function(arg_buff) {
			if(arg_buff < 100){
				$('#playing #playing_meta').html('<span>Buffering...' + arg_buff + '% </span>');
			}else{
	...
	DZ.Event.subscribe('track_end', function(arg) {
		...
		if(SmartPhone.isAny()){
			var take_action = confirm("Listening is limited to 30 second clips. To listen to unlimited music, Click OK to download or open your deezer app.");
			if(take_action == true){
				DZ.Event.subscribe('current_track', function(argct) {
					window.location.replace("http://www.deezer.com/track/" + argct.track.id);
				});
			}else{
				return false
			}
...
if(userAgent.match( /Android/i )){
	$('body').on('click', '.playlist-item .play, .playlist-l-item .play', function() {
		...
		$(this).parent().parent().find('.play').hide();
		addStyleAttribute($(this).parent().find('.play.flash-support'), 'display: none !important');
		addStyleAttribute($(this).parent().find('.play.no-flash'), 'display: none !important');
	...
	$('body').on('click', '.playlist-item .pause, .playlist-l-item .pause', function() {
		...
		addStyleAttribute($(this).parent().find('.play.flash-support'), 'display: block !important');
	...
	if('ontouchstart' in window || 'msmaxtouchpoints' in window.navigator){//check for touch device

		$('body').on('touchstart', '.playlist-item .cap-overlay .play, .playlist-l-item .cap-overlay .play', function(e) {
			var play_type_t = $(this).attr('play-type-t');
			//DZ.player.play();
			$('.play').show();
			$('.pause').hide();
			$(".cap-overlay-current").addClass('cap-overlay');
		...
		$('body').on('touchstart', '.playlist-item .pause, .playlist-l-item .pause', function() {
			DZ.player.pause();
	...
	************************************************************************/
	...
	function loadContent(target, toLoad) {
		$(target).load(toLoad, '', function(response, status, xhr) {
			...
			if('ontouchstart' in window || 'msmaxtouchpoints' in window.navigator){//check for touch device
			  	$('.playlist-item').on('touchstart', function(){
					$(".cap-overlay", this).fadeIn('slow');
				});
								
				$('.playlist-l-item').on('touchstart', function(){
					$(".cap-overlay", this).fadeIn('slow');
				});
